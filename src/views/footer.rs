...
//! secondary navigation router links, and external GitHub profile social anchors.
//!
//! ## Search Tags
//! #footer, #layout, #copyright, #navigation-links, #social-anchors

use crate::data::constants::{APP_TITLE, COPYRIGHT_YEAR, GITHUB_URL};
use crate::Route;
use dioxus::prelude::*;

#[component]
fn FooterCopyright() -> Element {
    rsx! {
...
fn FooterNavLinks() -> Element {
    rsx! {
        nav { class: "flex flex-wrap justify-center gap-8 text-sm font-semibold text-text-dark/70 dark:text-[#D4D4D4] order-1 md:order-2",
            Link { class: "hover:text-primary-light transition-all hover:-translate-y-0.5", to: Route::Home {}, "Home" }
            Link { class: "hover:text-primary-light transition-all hover:-translate-y-0.5", to: Route::BlogList {}, "Blog" }
            Link { class: "hover:text-primary-light transition-all hover:-translate-y-0.5", to: Route::ProjectList {}, "Projects" }
            Link { class: "hover:text-primary-light transition-all hover:-translate-y-0.5", to: Route::About {}, "About" }
            Link { class: "hover:text-primary-light transition-all hover:-translate-y-0.5", to: Route::Contact {}, "Contact" }
        }
    }
}

#[component]
fn GithubFooterIcon() -> Element {
    rsx! {
        a {
...
/// Global page footer presenting legal copyright, navigation links, and social connections.
#[component]
