    ...
    rsx! {
        div { class: "w-full max-w-2xl bg-surface-dark dark:bg-background-darker border border-surface-border dark:border-white/10 rounded-xl shadow-2xl overflow-hidden mb-12",
            ...
            div { class: "p-6 md:p-10 font-mono text-sm md:text-base leading-relaxed text-gray-300",
                ...
                div { class: "text-gray-500 mb-4 select-none", "   Compiling router v0.1.0 (/usr/src/rustdev)" }
                div { class: "text-gray-500 mb-4 select-none", "    Finished dev [unoptimized + debuginfo] target(s) in 0.42s" }
                div { class: "text-gray-500 mb-4 select-none", "     Running `target/debug/router`" }
                div { class: "mt-6 text-red-500 font-bold", "thread 'main' panicked at 'Page not found: 404', src/main.rs:42:5" }
                div { class: "text-gray-500 mt-1", "note: run with `RUST_BACKTRACE=1` environment variable to display a backtrace" }
...
/// Fallback route view rendered when accessing undefined URL segments.
#[component]
